...
\end{minted}
\caption{Extracto de Dockerfile utilizado en el proyecto}
...
volumes:
  mongo_data:
\end{minted}
\caption{docker-compose.yaml usado en el proyecto}
\label{lst:dockercompose}
\end{longlisting}

Utilizando el comando del Listing \ref{lst:dcup}, y teniendo encuenta el Listing \ref{lst:dockercompose}:
...
\end{minted}
\caption{Archivo 'kustomization.yaml'}
    ...
    steps:
      ...
      - name: Push New Files
        ...
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
  \end{minted}
  \caption{Workflow de publicación de la Chart en GH Pages}
  \label{lst:ghpages}
\end{longlisting}
